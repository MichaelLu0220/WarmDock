...
import type { UnlockNodeState } from "../../core/types";
...
function getNodeColors(
  ...
  } else if (!state.requirementsMet || !isRootAwakened) {
    fill = "var(--wd-paper)";
...
export function UnlockNode({
  ...
  const dimIcon =
    !state.unlocked && (!state.requirementsMet || !isRootAwakened);
  ...
  return (
    <g
      data-node-id={nodeId}
      className={pulse ? "wd-node--purchasable" : undefined}
        ...
        <>
          <defs>
            <clipPath id={`wd-hold-clip-${nodeId}`}>
              <rect x={nodeLeft} y={nodeTop} width={size} height={size} />
            </clipPath>
          </defs>
          <rect
            x={nodeLeft}
