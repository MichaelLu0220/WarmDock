...
/**
 * 「翻書」頁切換轉場(通用版,沿用 demo 的縮放 morph + 多頁 3D 翻入)。
 *
 * 監看 `appPage`:索引變大 = 往前翻(下一頁,fwd)、變小 = 往回翻(上一頁,back)。
 * 卡片一律呈「書的左半頁」、書脊在右緣;切換時在卡上疊一層 3D 書(fixed),
 * 底下卡片瞬切到新頁並被書蓋住(整張卡 visibility:hidden),翻完才顯露。
 *
 * 容器的「source→中央輕縮→dest」位移/縮放用 WAAPI(rect 動態),每頁翻動用
 * CSS(stagger delay)。CSS 在 ui-web globals.css 的「翻書轉場」段,時間軸常數
 * 需與此檔對齊。demo 的 DemoBookFlip 是同套機制、改綁 allTasksCompleted。
 */
const PAGE_COUNT = 6;
...
export function BookFlip({
  cardSelector = ".wd-web-stage .wd-panel.wd-card",
}: {
  cardSelector?: string;
}) {
  const appPage = useUIStore((s) => s.appPage);
  const prevRef = useRef(appPage);
  ...
  // 偵測 appPage 切換:在繪製前疊書、藏卡(避免新頁先閃一格)。
  useLayoutEffect(() => {
    if (appPage === prevRef.current) return;
    const dir: "fwd" | "back" = appPage > prevRef.current ? "fwd" : "back";
    prevRef.current = appPage;
    ...
    const src = lastRect.current ?? dest; // 上一次 commit 記下的尺寸 = 來源頁
